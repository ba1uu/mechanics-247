...
export default function Home() {
  ...
  // Reverse geocode real coordinates → human-readable address
  useEffect(() => {
    if (geo.status === "success" && geo.lat && geo.lng) {
      setLocationLabel("📍 Fetching your address...");
      fetch(
        ...
        .then(data => {
          ...
          setLocationLabel("📍 " + (parts.join(", ") || data.display_name?.split(",").slice(0, 2).join(",") || "Location detected"));
        })
        .catch(() => setLocationLabel(`📍 ${geo.lat.toFixed(4)}, ${geo.lng.toFixed(4)}`));
    } else if (geo.status === "error") {
      setLocationLabel("⚠️ Location access denied — click to retry");
    } else {
    ...
    <div style={{ background: C.cream, overflowX: "hidden" }}>
        ...
        <div className="hero-grid" style={{ maxWidth: 1280, margin: "0 auto", padding: "120px 60px 80px", width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "center", position: "relative", zIndex: 2 }}>
          <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? "translateY(0)" : "translateY(40px)", transition: "all .8s ease" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(224,123,26,.1)", border: `1px solid rgba(224,123,26,.25)`, borderRadius: 100, padding: "7px 18px", marginBottom: 32 }}>
              ...
              <span style={{ fontFamily: "'Oswald',sans-serif", fontSize: 11, fontWeight: 600, color: C.amberDark, letterSpacing: 1.5 }}>NOW IN BETA · LAUNCHING SOON</span>
            </div>
            ...
            <div style={{ display: "flex", gap: 24, paddingTop: 32, borderTop: `1px solid rgba(224,123,26,.15)`, flexWrap: "wrap" }}>
              {[
                { val: "8%", label: "PLATFORM FEE" },
                { val: "Daily", label: "PAYOUTS" },
                { val: "24/7", label: "AVAILABLE" },
                { val: "< 8 min", label: "AVG RESPONSE" },
              ].map((s, i) => (
                <div key={i}>
                  <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 26, fontWeight: 700, color: C.amberDark, lineHeight: 1 }}>{s.val}</div>
            ...
            <div className="hero-card" style={{ background: C.white, border: `1px solid rgba(224,123,26,.15)`, borderRadius: 24, padding: 32, boxShadow: "0 32px 80px rgba(92,46,10,.12)", position: "relative" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
                <input
                  ...
                  style={{
                    padding: "11px 14px",
                    border: `1.5px solid ${geo.status === "success" ? C.amber : C.border}`,
                    borderRadius: 8,
                    fontSize: 13,
                    background: geo.status === "success" ? "#fffbf5" : C.cream,
                    width: "100%",
                    cursor: geo.status === "success" ? "default" : "pointer",
                    boxSizing: "border-box",
                    color: geo.status === "success" ? C.textPrimary : C.textMuted,
                    fontWeight: geo.status === "success" ? 500 : 400,
                  }}
                />
              ...
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontSize: 12, color: C.textMuted }}>
                ...
                <span><strong style={{ color: C.green }}>Founding mechanic slots</strong> open across India</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ background: C.brown, padding: "20px 60px" }}>
        <div className="trust-bar" style={{ maxWidth: 1280, margin: "0 auto", display: "flex", gap: 40, justifyContent: "center", alignItems: "center", flexWrap: "wrap" }}>
          {["🔒 100% Verified Mechanics", "⚡ Avg 8 Min Response", "💳 Secure UPI Payments", "📱 Available 24/7", "💰 Only 8% Commission", "🇮🇳 Made in India"].map(t => (
        ...
        <div className="mechanic-grid" style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <div>
            ...
            <h2 style={{ fontFamily: "'Oswald',sans-serif", fontSize: 44, fontWeight: 700, color: C.textPrimary, marginBottom: 16, lineHeight: 1.05 }}>
              Set your own hours, earn on your <span style={{ color: C.amber }}>own terms</span>
            </h2>
            <p style={{ fontSize: 16, color: C.textSecondary, lineHeight: 1.8, marginBottom: 32 }}>
              Join our growing network of mechanics earning on their own terms. Set your own hours, service area, and availability.
            </p>
          ...
          <div className="mechanic-cards" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {[
              { label: "Platform Commission", val: "8%", icon: "📊", color: C.amber, sub: "Lowest in the industry" },
              { label: "Payout Schedule", val: "Daily", icon: "💰", color: C.orange, sub: "Money in account, same day" },
              { label: "Job Acceptance", val: "1 Tap", icon: "📱", color: C.terra, sub: "See job, accept, go" },
              { label: "Working Hours", val: "Yours", icon: "⏰", color: C.amberDark, sub: "Work when you want" },
            ].map((m, i) => (
      ...
      <section style={{ padding: "100px 60px", background: C.cream2 }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "'Oswald',sans-serif", fontSize: 48, fontWeight: 700, color: C.textPrimary, marginBottom: 16 }}>Real stories, <span style={{ color: C.amber }}>coming soon</span></h2>
          <p style={{ fontSize: 16, color: C.textSecondary, marginBottom: 40, maxWidth: 480, margin: "0 auto 40px" }}>
            We&apos;re in beta — real customer reviews will appear here after launch. Be the first to share yours.
          </p>
          <button
            onClick={() => router.push("/register")}
            style={{ padding: "14px 36px", background: `linear-gradient(135deg, ${C.amber}, ${C.orange})`, color: "white", border: "none", borderRadius: 10, fontFamily: "'Oswald',sans-serif", fontSize: 16, fontWeight: 600, letterSpacing: 1, cursor: "pointer" }}
          >
      ...
      <section style={{ padding: "80px 60px", background: C.white }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          <span style={{ fontFamily: "'Oswald',sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: 3, color: C.amber, display: "block", marginBottom: 10 }}>FROM THE FOUNDER</span>
          <h2 style={{ fontFamily: "'Oswald',sans-serif", fontSize: 40, fontWeight: 700, color: C.textPrimary, marginBottom: 32 }}>Why we <span style={{ color: C.amber }}>built this</span></h2>
          <div style={{ background: C.cream, border: `1px solid ${C.border}`, borderRadius: 20, padding: 36 }}>
            <div style={{ color: C.amber, fontSize: 20, letterSpacing: 3, marginBottom: 16 }}>★★★★★</div>
            <p style={{ fontSize: 17, color: C.textSecondary, lineHeight: 1.75, marginBottom: 28, fontStyle: "italic" }}>
              &ldquo;I was stranded on a highway at midnight with no help in sight. That moment made me realise India needed a reliable, honest roadside assistance platform — one that treats both customers and mechanics fairly. That&apos;s why I built Mechanics 24/7.&rdquo;
            </p>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ width: 56, height: 56, borderRadius: "50%", background: `linear-gradient(135deg, ${C.amber}, ${C.terra})`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Oswald',sans-serif", fontSize: 20, fontWeight: 700, color: "white", flexShrink: 0 }}>SC</div>
              <div>
                <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 16, fontWeight: 700, color: C.textPrimary }}>Sai Charan</div>
                <div style={{ fontSize: 13, color: C.amber, fontWeight: 600, marginTop: 2 }}>Founder, Mechanics 24/7</div>
                <div style={{ fontSize: 12, color: C.textMuted, marginTop: 2 }}>Building from India 🇮🇳</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="footer-grid" style={{ display: "grid", gridTemplateColumns: "2.5fr 1fr 1fr 1fr 1fr", gap: 40, marginBottom: 48 }}>
            <div className="footer-brand">
              ...
              <div style={{ marginTop: 16, padding: "10px 14px", background: "rgba(245,158,42,.08)", border: "1px solid rgba(245,158,42,.15)", borderRadius: 8 }}>
                ...
                <div style={{ fontSize: 12, color: "rgba(255,255,255,.6)", fontWeight: 600 }}>Sai Charan</div>
              </div>
            </div>
            {[
              ...
              { heading: "COMPANY", links: [{ l: "About Us", h: "/about" }, { l: "Contact", h: "/contact" }, { l: "Careers", h: "/about" }] },
